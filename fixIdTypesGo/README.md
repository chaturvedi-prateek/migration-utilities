...
| File | Description |
|---|---|
| `fix-id-types-linux-amd64` | Linux x86-64 static binary |
| `fix-id-types-windows-amd64.exe` | Windows x86-64 static binary |
...
```
fix-id-types --mode <detect|count|fix> [--config config.json] [--dry-run=true]
```
...
```bash
# Linux
./fix-id-types-linux-amd64 --mode detect --config config.json

# Windows (PowerShell)
.\fix-id-types-windows-amd64.exe --mode detect --config config.json
```
...
```bash
# Linux
./fix-id-types-linux-amd64 --mode count --config config.json

# Windows (PowerShell)
.\fix-id-types-windows-amd64.exe --mode count --config config.json
```
...
```bash
# Linux
./fix-id-types-linux-amd64 --mode fix --config config.json --dry-run=true

# Windows (PowerShell)
.\fix-id-types-windows-amd64.exe --mode fix --config config.json --dry-run=true
```
...
```bash
# Linux
./fix-id-types-linux-amd64 --mode fix --config config.json --dry-run=false

# Windows (PowerShell)
.\fix-id-types-windows-amd64.exe --mode fix --config config.json --dry-run=false
```
...
# Current platform
go build -o fix-id-types .

# Linux
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o fix-id-types-linux-amd64 .

# Windows
CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -ldflags="-s -w" -o fix-id-types-windows-amd64.exe .
```
