...
The script checks the last 500 entries of the dump. If the replay stopped very early (more than 500 entries from the end), the script may not detect the exact boundary. Re-run with a larger tail by increasing the `tail = entries[-500:]` value in `oplogResumeLocal.py` line 58.
...
python3 oplogResumeLocal.py --dump oplog.bson --generate --js find_resume.js
...
python3 oplogResumeLocal.py --dump oplog.bson --filter --t <T> --i <I> --out ./resume_dir
