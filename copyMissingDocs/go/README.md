...
| File | Description |
|---|---|
| `bin/copyMissingDocs-darwin-arm64` | macOS (Apple Silicon) binary |
| `bin/copyMissingDocs-darwin-amd64` | macOS (Intel) binary |
| `bin/copyMissingDocs-linux-amd64` | Linux x86-64 binary |
| `bin/copyMissingDocs-linux-arm64` | Linux ARM64 binary |
| `bin/copyMissingDocs-windows-amd64.exe` | Windows x86-64 binary |
...
**Linux:**
```bash
chmod +x bin/copyMissingDocs-linux-amd64
./bin/copyMissingDocs-linux-amd64 --config config.json 2>&1 | tee copyMissingDocs-dryrun-$(date +%Y%m%d-%H%M%S).log
```

**Windows (PowerShell):**
```powershell
.\bin\copyMissingDocs-windows-amd64.exe --config config.json 2>&1 | Tee-Object -FilePath copyMissingDocs-dryrun.log
```

**Windows (CMD):**
```cmd
bin\copyMissingDocs-windows-amd64.exe --config config.json > copyMissingDocs-dryrun.log 2>&1
type copyMissingDocs-dryrun.log
```
...
**Linux:**
```bash
./bin/copyMissingDocs-linux-amd64 --config config.json --dry-run=false 2>&1 | tee copyMissingDocs-live-$(date +%Y%m%d-%H%M%S).log
```

**Windows (PowerShell):**
```powershell
.\bin\copyMissingDocs-windows-amd64.exe --config config.json --dry-run=false 2>&1 | Tee-Object -FilePath copyMissingDocs-live.log
```

**Windows (CMD):**
```cmd
bin\copyMissingDocs-windows-amd64.exe --config config.json --dry-run=false > copyMissingDocs-live.log 2>&1
type copyMissingDocs-live.log
```
...
```
============================================================
  copyMissingDocs  |  DRY RUN — no changes will be written
...
go build -o copyMissingDocs .

# Cross-compile all common platforms into bin/
CGO_ENABLED=0 GOOS=darwin  GOARCH=arm64 go build -ldflags="-s -w" -o bin/copyMissingDocs-darwin-arm64 .
CGO_ENABLED=0 GOOS=darwin  GOARCH=amd64 go build -ldflags="-s -w" -o bin/copyMissingDocs-darwin-amd64 .
CGO_ENABLED=0 GOOS=linux   GOARCH=amd64 go build -ldflags="-s -w" -o bin/copyMissingDocs-linux-amd64 .
CGO_ENABLED=0 GOOS=linux   GOARCH=arm64 go build -ldflags="-s -w" -o bin/copyMissingDocs-linux-arm64 .
CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -ldflags="-s -w" -o bin/copyMissingDocs-windows-amd64.exe .
```
