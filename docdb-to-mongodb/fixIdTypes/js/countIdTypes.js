...
(async () => {
  ...
  print(SEPARATOR);
  logInfo(`countIdTypes — _id type distribution for ${NAMESPACES.length} namespace(s)`);
  ...
  for (const ns of NAMESPACES) {
    ...
    try {
      ...
      if (rows.length > 1) {
        logInfo("  STATUS : MIXED — configure fixIdTypes.js and run to fix before migration");
      } else {
