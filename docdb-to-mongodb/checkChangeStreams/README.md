...
| Platform | Binary |
| --- | --- |
| macOS (Apple Silicon) | `bin/checkChangeStreams-darwin-arm64` |
| macOS (Intel) | `bin/checkChangeStreams-darwin-amd64` |
| Linux (x86_64) | `bin/checkChangeStreams-linux-amd64` |
| Linux (ARM64) | `bin/checkChangeStreams-linux-arm64` |
| Windows (x86_64) | `bin/checkChangeStreams-windows-amd64.exe` |

**Linux:**

```bash
chmod +x bin/checkChangeStreams-linux-amd64
./bin/checkChangeStreams-linux-amd64 -ca-file global-bundle.pem
```

**Windows (PowerShell):**

```powershell
.\bin\checkChangeStreams-windows-amd64.exe -ca-file global-bundle.pem
```
...
```bash
go mod tidy
go build -o checkChangeStreams .
```

To reproduce the cross-platform binaries yourself (output into `bin/`):

```bash
GOOS=darwin  GOARCH=arm64 CGO_ENABLED=0 go build -o bin/checkChangeStreams-darwin-arm64 .
GOOS=darwin  GOARCH=amd64 CGO_ENABLED=0 go build -o bin/checkChangeStreams-darwin-amd64 .
GOOS=linux   GOARCH=amd64 CGO_ENABLED=0 go build -o bin/checkChangeStreams-linux-amd64 .
GOOS=linux   GOARCH=arm64 CGO_ENABLED=0 go build -o bin/checkChangeStreams-linux-arm64 .
GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -o bin/checkChangeStreams-windows-amd64.exe .
```
...
```bash
./checkChangeStreams -ca-file global-bundle.pem \
...
./checkChangeStreams -ca-file global-bundle.pem -db mydb

# Machine-readable JSON
./checkChangeStreams -ca-file global-bundle.pem -json
```
