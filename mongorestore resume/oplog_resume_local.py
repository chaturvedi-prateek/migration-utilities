#!/usr/bin/env python3
"""
oplog_resume_local.py — Runs on LOCAL machine (needs only: pip install pymongo)

Two modes:

  Mode 1 — Generate a mongosh script to find the resume point on the server:
    python3 oplog_resume_local.py --dump oplog.bson --generate

    → Produces find_resume.js  (copy this to the server and run with mongosh)

  Mode 2 — Filter the dump from a known resume point:
    python3 oplog_resume_local.py --dump oplog.bson --filter --t 1718500000 --i 1 --out ./resume_dir
