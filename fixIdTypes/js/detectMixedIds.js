...
(async () => {
  ...
  print(SEPARATOR);
  logInfo("detectMixedIds — fast _id type scan via ASC / DESC index probes");
  ...
  if (mixedList.length > 0) {
    ...
    print(SEPARATOR2);
    logInfo("Next steps:");
    logInfo("  1. Run countIdTypes.js on the above namespaces for exact counts per type");
    logInfo("  2. Add the namespaces to fixIdTypes.js COLLECTIONS array and run to fix");
  } else {
