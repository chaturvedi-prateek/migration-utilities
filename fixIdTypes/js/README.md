...
|---|---|---|
| `detectMixedIds.js` | Scan databases — find collections with mixed `_id` types | Fast — 2 index seeks per collection |
| `countIdTypes.js` | For flagged namespaces — count documents per `_id` type | Fast — covered index scan |
| `fixIdTypes.js` | Fix wrong `_id` types (backup → insert corrected → delete original) | Full collection scan (writes) |
...
## Step 1 — Detect: `detectMixedIds.js`
...
Open `detectMixedIds.js` and set `DATABASES`:
...
mongosh "mongodb://user:pass@docdb-cluster.ap-south-1.docdb.amazonaws.com:27017" `
  ...
  --quiet `
  --file detectMixedIds.js | Out-File -Encoding utf8 detectMixedIds.log
```
...
mongosh "mongodb://user:pass@docdb-cluster.ap-south-1.docdb.amazonaws.com:27017" \
  ...
  --readPreference secondaryPreferred \
  --quiet \
  --file detectMixedIds.js > detectMixedIds.log
```
...
```
════════════════════════════════════════════════════════════
[INFO ] detectMixedIds — fast _id type scan via ASC / DESC index probes
...
[INFO ] Next steps:
[INFO ]   1. Run countIdTypes.js on the above namespaces for exact counts per type
[INFO ]   2. Add the namespaces to fixIdTypes.js COLLECTIONS array and run to fix
[INFO ]   Elapsed : 1.4s
════════════════════════════════════════════════════════════
```
...
Open `countIdTypes.js` and set `NAMESPACES` (paste the mixed namespaces from the detect output):
...
mongosh "mongodb://user:pass@docdb-cluster.ap-south-1.docdb.amazonaws.com:27017" `
  ...
  --readPreference secondaryPreferred `
  --quiet `
  --file countIdTypes.js | Out-File -Encoding utf8 countIdTypes.log
```
...
mongosh "mongodb://user:pass@docdb-cluster.ap-south-1.docdb.amazonaws.com:27017" \
  ...
  --readPreference secondaryPreferred \
  --quiet \
  --file countIdTypes.js > countIdTypes.log
```
...
```
════════════════════════════════════════════════════════════
[INFO ] countIdTypes — _id type distribution for 2 namespace(s)
────────────────────────────────────────────────────────────
...
[INFO ]   STATUS : MIXED — configure fixIdTypes.js and run to fix before migration
────────────────────────────────────────────────────────────
...
Open `fixIdTypes.js` and update `COLLECTIONS`. The script is pre-configured for two known MSIL collections:
...
mongosh "mongodb://user:pass@docdb-cluster.ap-south-1.docdb.amazonaws.com:27017" `
  ...
  --readPreference secondaryPreferred `
  --quiet `
  --file fixIdTypes.js | Out-File -Encoding utf8 fixIdTypes_dryrun.log
```
...
mongosh "mongodb://user:pass@docdb-cluster.ap-south-1.docdb.amazonaws.com:27017" \
  ...
  --readPreference secondaryPreferred \
  --quiet \
  --file fixIdTypes.js > fixIdTypes_dryrun.log
```
